...
export default async function ServicePage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params; const service = getService(slug); if (!service) notFound(); const Icon = service.icon;
  ...
  return (
    <>
      ...
      <section className="border-y border-border bg-muted/55 py-16 md:py-22"><div className="container-page"><p className="eyebrow">How it works</p><h2 className="section-title mt-5">Care begins with the right level of assessment.</h2><div className="mt-10 grid gap-4 md:grid-cols-4">{[{t:"Request",d:"Share only essential booking details."},{t:"Clarify",d:"The clinic confirms time and visit format."},{t:"Assess",d:"Clinical needs and safety guide recommendations."},{t:"Review",d:"Progress and next steps stay transparent."}].map((step,index)=><div key={step.t} className="rounded-2xl border border-border bg-card p-5"><span className="text-xs font-black text-primary/55">0{index+1}</span><h3 className="mt-3 font-extrabold">{step.t}</h3><p className="mt-2 text-sm leading-6 text-muted-foreground">{step.d}</p></div>)}</div></div></section>
      <section className="container-page py-16 md:py-20"><h2 className="text-2xl font-extrabold tracking-[-0.035em]">Explore related services</h2><div className="mt-7 grid gap-4 md:grid-cols-3">{others.map((item)=><Link key={item.slug} href={`/services/${item.slug}`} className="group rounded-2xl border border-border bg-card p-5 shadow-card transition hover:-translate-y-1 hover:border-accent/35"><p className="font-extrabold group-hover:text-primary">{item.name}</p><p className="mt-2 text-sm leading-6 text-muted-foreground">{item.short}</p><span className="mt-4 inline-flex items-center gap-1 text-xs font-extrabold text-primary">Learn more<ArrowRight className="size-3.5 transition group-hover:translate-x-1"/></span></Link>)}</div><div className="mt-7 flex items-center gap-3 rounded-2xl border border-border bg-card p-5 text-sm leading-6 text-muted-foreground"><ShieldCheck className="size-5 shrink-0 text-secondary"/>Information is educational. A service is recommended only after assessing individual need and safety.</div></section>
