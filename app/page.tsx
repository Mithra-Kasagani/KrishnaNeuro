...
import { articleImage, conditionImage } from "@/lib/page-images";
import { siteConfig } from "@/lib/site";

export const metadata: Metadata = createMetadata({
  title: "Psychiatrist in Vijayawada | Dr. Pamarthi Krishna Das",
  description: "Consult Dr. Pamarthi Krishna Das, MBBS, MD (Psychiatry), at Krishna Neuro Psychiatric Centre in Suryaraopet, Vijayawada. Hopeful, ethical mental-health care.",
  path: "/",
  keywords: ["psychiatrist in Vijayawada", "Dr Pamarthi Krishna Das", "Krishna Neuro Psychiatric Centre", "mental health clinic Vijayawada", "psychiatrist Suryaraopet"],
});
  ...
  return (
    <>
      <section className="relative overflow-hidden border-b border-border bg-card">
        <Image src="/illustrations/hero-aurora.svg" alt="" fill priority unoptimized sizes="100vw" aria-hidden="true" className="object-cover opacity-60 dark:opacity-20" />
        ...
        <div className="container-page relative grid min-h-[calc(100svh-7.25rem)] items-center gap-12 py-10 lg:grid-cols-[1.08fr_.92fr] lg:py-16">
          <div className="relative z-10 max-w-3xl">
            <div>
              ...
              <h1 className="display-title mt-4 max-w-3xl text-foreground">Mental health care that sees the <span className="relative text-primary"><span className="relative z-10">person</span><span className="absolute inset-x-0 bottom-1.5 -z-0 h-3 rounded-full bg-accent/16" aria-hidden="true"/></span>, not just the symptoms.</h1>
              <p className="mt-7 max-w-2xl text-pretty text-lg leading-8 text-muted-foreground md:text-xl">Compassionate, evidence-based psychiatric care with <strong className="font-bold text-foreground">Dr. Pamarthi Krishna Das</strong> in Vijayawada—for individuals, children, older adults and families.</p>
              <div className="mt-8 flex flex-wrap gap-3">
                <Button asChild size="lg"><a href="/appointment"><CalendarDays aria-hidden="true"/>Request appointment<ArrowRight aria-hidden="true"/></a></Button>
                <Button asChild variant="outline" size="lg"><a href={`tel:${siteConfig.phones[0]}`}><Phone aria-hidden="true"/>Call {siteConfig.displayPhones[0]}</a></Button>
              </div>
          ...
          <div className="pointer-events-none absolute inset-y-0 right-0 w-[72%] max-w-none opacity-20 [mask-image:linear-gradient(to_right,transparent,black_55%)] lg:pointer-events-auto lg:relative lg:inset-auto lg:mr-0 lg:w-full lg:max-w-[35rem] lg:opacity-100 lg:[mask-image:none]">
            ...
            <div className="relative h-full overflow-hidden bg-slate-100 lg:h-auto lg:rounded-[2.2rem] lg:border lg:border-white/50 lg:shadow-[0_32px_75px_-34px_rgb(15_76_129/.48)] dark:border-white/10 dark:bg-slate-800">
              <Image src="/images/doctor-portrait.webp" alt="Dr. Pamarthi Krishna Das, Consultant Psychiatrist, at Krishna Neuro Psychiatric Centre in Vijayawada" width={960} height={1160} priority sizes="(max-width: 1024px) 90vw, 42vw" className="h-full w-full object-cover object-top lg:aspect-[4/4.7] lg:h-auto" />
      ...
      <section className="container-page py-20 md:py-28">
        <div className="grid items-center gap-12 lg:grid-cols-[.86fr_1.14fr] lg:gap-18">
          <Reveal className="relative">
            ...
            <div className="absolute -bottom-5 -right-2 max-w-[15rem] rounded-2xl border border-border bg-card p-4 shadow-xl sm:right-5"><p className="text-[0.68rem] font-extrabold uppercase tracking-[0.12em] text-primary">A simple belief</p><p className="mt-2 text-sm font-bold leading-6 text-foreground">“Every patient deserves dignity, hope and a plan they can understand.”</p></div>
          </Reveal>
          <Reveal delay={0.08}>
            <p className="eyebrow">Meet your psychiatrist</p>
            <h2 className="section-title mt-5 text-balance">Care built around careful listening—not assumptions.</h2>
            ...
            <div className="mt-7 flex flex-wrap gap-3"><Button asChild variant="soft"><a href="/about-doctor">About Dr. Krishna Das<ArrowRight aria-hidden="true"/></a></Button><Button asChild variant="ghost"><a href="/treatments">Our treatment approach</a></Button></div>
          </Reveal>
        </div>
      </section>

      <section className="border-y border-border bg-muted/55 py-20 md:py-28">
        <div className="container-page">
          <Reveal><SectionHeading eyebrow="Conditions we treat" title={<>From the first difficult symptom to a clearer way forward.</>} description="Explore plain-language guides to common concerns. These pages educate; an individual diagnosis still requires a consultation." /></Reveal>
      ...
      <section className="relative overflow-hidden bg-[#0b365c] py-20 text-white md:py-28">
      ...
      <section className="border-y border-border bg-card py-14">
        <div className="container-page grid grid-cols-2 gap-px overflow-hidden rounded-[1.8rem] border border-border bg-border lg:grid-cols-4">
          {[{ value: `${conditions.length}`, label: "condition guides" }, { value: "10", label: "care services" }, { value: "7", label: "clear journey steps" }, { value: "9–9", label: "listed Mon–Sat hours" }].map((stat) => <div key={stat.label} className="bg-card p-6 text-center md:p-8"><p className="text-3xl font-extrabold tracking-[-0.05em] text-primary md:text-4xl">{stat.value}</p><p className="mt-2 text-xs font-bold uppercase tracking-[0.1em] text-muted-foreground">{stat.label}</p></div>)}
        </div>
        <p className="container-page mt-4 text-center text-[0.68rem] text-muted-foreground">These are service and information counts—not treatment-outcome claims. Timings should be confirmed by phone.</p>
      </section>

      <section className="container-page py-20 md:py-28">
        <Reveal><SectionHeading eyebrow="What respectful care can feel like" title="Care experiences, without the marketing gloss." description="These are clearly labelled composite scenarios illustrating care values. They are not patient reviews and are never used as review schema." /></Reveal>
        <Reveal delay={0.08} className="mt-10"><PatientStories /></Reveal>
        <div className="mt-8"><GoogleReviews /></div>
      </section>

      <GallerySection compact className="bg-muted/45" />

      <section className="border-y border-border bg-muted/55 py-20 md:py-28">
        <div className="container-page grid gap-12 lg:grid-cols-[.8fr_1.2fr]">
          <Reveal><SectionHeading eyebrow="Questions before the first visit" title="Clear answers can reduce uncertainty." description="Start with the questions patients and families ask most often." /><Button asChild variant="outline" className="mt-7"><a href="/faqs">View all FAQs<ArrowRight aria-hidden="true"/></a></Button></Reveal>
      ...
      <section className="container-page py-20 md:py-28">
        <Reveal><SectionHeading eyebrow="Mental health library" title="Useful knowledge, written for real life." description="Simple, responsible guides for recognising patterns, supporting family and knowing when to seek care." /></Reveal>
        <Stagger className="mt-10 grid gap-5 md:grid-cols-3">
          {articles.slice(0, 3).map((article) => <StaggerItem key={article.slug}><a href={`/blog/${article.slug}`} className="group block h-full overflow-hidden rounded-[1.6rem] border border-border bg-card shadow-card transition duration-300 hover:-translate-y-1 hover:border-accent/35 hover:shadow-soft"><div className="relative aspect-[16/9]"><Image src={articleImage(article.slug)} alt={`Editorial photograph for ${article.title}`} fill sizes="(max-width: 768px) 100vw, 33vw" className="object-cover transition duration-500 group-hover:scale-[1.03]"/></div><div className="p-6"><Badge>{article.category}</Badge><h3 className="mt-5 text-xl font-extrabold leading-tight tracking-[-0.03em] group-hover:text-primary">{article.title}</h3><p className="mt-3 line-clamp-3 text-sm leading-6 text-muted-foreground">{article.description}</p><div className="mt-6 flex items-center justify-between text-xs font-bold text-muted-foreground"><span>{article.readingMinutes} min read</span><span className="inline-flex items-center gap-1 text-primary">Read guide<ArrowRight className="size-3.5 transition group-hover:translate-x-1" aria-hidden="true"/></span></div></div></a></StaggerItem>)}
        </Stagger>
        <div className="mt-8 text-center"><Button asChild variant="outline"><a href="/blog">Browse all articles<ArrowRight aria-hidden="true"/></a></Button></div>
      </section>
      ...
      <section className="container-page py-12 md:py-16"><EmergencyNote /></section>
