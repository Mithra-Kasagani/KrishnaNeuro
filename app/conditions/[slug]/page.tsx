...
export default async function ConditionPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <>
      <MedicalWebPageJsonLd name={`${condition.name}: Symptoms, Diagnosis and Treatment`} description={condition.summary} path={`/conditions/${condition.slug}`} about={condition.name} />
      <PageHero
        badge={condition.category}
        image={conditionImage(condition.slug)}
        imageAlt={aiImageAlt(condition.name)}
        title={<>{condition.name}: understand what is happening, without losing hope.</>}
        description={<><p>{condition.summary}</p>{condition.ageNote && <p className="mt-4 text-sm font-semibold text-foreground">{condition.ageNote}</p>}</>}
      ...
      <section id="symptoms" className="container-page py-16 md:py-22">
        <div className="grid gap-10 lg:grid-cols-[.78fr_1.22fr] lg:gap-18">
          <Reveal><div className="sticky top-30"><SectionHeading eyebrow="Symptoms" title="What you or your family may notice" description="One sign alone does not establish a diagnosis. Pattern, duration, severity, context and impact all matter."/></div></Reveal>
      ...
      <section className="border-y border-border bg-muted/55 py-16 md:py-22">
        <div className="container-page grid gap-6 lg:grid-cols-2">
          <Reveal><Card className="h-full p-7"><span className="flex size-11 items-center justify-center rounded-2xl bg-primary/8 text-primary"><SearchCheck className="size-5"/></span><h2 className="mt-5 text-2xl font-extrabold tracking-[-0.035em]">Common causes and contributors</h2><p className="mt-3 text-sm leading-6 text-muted-foreground">Usually there is no single cause and no one deserves blame.</p><ul className="mt-6 grid gap-3">{condition.causes.map((item)=><li key={item} className="flex gap-3 text-sm leading-6 text-muted-foreground"><span className="mt-2 size-1.5 shrink-0 rounded-full bg-accent"/>{item}</li>)}</ul></Card></Reveal>
          <Reveal delay={0.08}><Card className="h-full p-7"><span className="flex size-11 items-center justify-center rounded-2xl bg-secondary/10 text-secondary"><ShieldCheck className="size-5"/></span><h2 className="mt-5 text-2xl font-extrabold tracking-[-0.035em]">Risk factors</h2><p className="mt-3 text-sm leading-6 text-muted-foreground">A risk factor raises likelihood; it does not make an outcome inevitable.</p><ul className="mt-6 grid gap-3">{condition.riskFactors.map((item)=><li key={item} className="flex gap-3 text-sm leading-6 text-muted-foreground"><span className="mt-2 size-1.5 shrink-0 rounded-full bg-secondary"/>{item}</li>)}</ul></Card></Reveal>
        </div>
      </section>

      <section className="container-page py-16 md:py-22">
        <div className="grid items-start gap-10 lg:grid-cols-[.8fr_1.2fr] lg:gap-18">
          <Reveal><div className="rounded-[1.8rem] bg-[#0b3b64] p-7 text-white shadow-soft"><ClipboardCheck className="size-7 text-emerald-300"/><h2 className="mt-5 text-3xl font-extrabold tracking-[-0.04em]">How diagnosis works</h2><p className="mt-5 text-sm leading-7 text-slate-200">{condition.diagnosis}</p><p className="mt-5 border-t border-white/15 pt-5 text-xs leading-6 text-slate-300">Online tests may support reflection but cannot replace a clinical history, risk assessment and relevant medical evaluation.</p></div></Reveal>
          <Reveal delay={0.08}><SectionHeading eyebrow="Treatment options" title="A personalised plan may combine several approaches." description="Recommendations depend on severity, physical health, previous response, preferences, access and safety."/><div className="mt-8 grid gap-4">{condition.treatments.map((treatment,index)=><div key={treatment.title} className="grid gap-3 rounded-2xl border border-border bg-card p-5 shadow-card sm:grid-cols-[auto_1fr]"><span className="flex size-9 items-center justify-center rounded-xl bg-primary/8 text-xs font-extrabold text-primary">0{index+1}</span><div><h3 className="font-extrabold text-foreground">{treatment.title}</h3><p className="mt-1.5 text-sm leading-6 text-muted-foreground">{treatment.description}</p></div></div>)}</div></Reveal>
        </div>
      </section>

      <section className="border-y border-border bg-card py-16 md:py-22">
        <div className="container-page grid gap-8 lg:grid-cols-2">
          <Reveal><div className="h-full rounded-[1.8rem] border border-border bg-muted/55 p-7"><CircleAlert className="size-6 text-primary"/><h2 className="mt-5 text-2xl font-extrabold tracking-[-0.035em]">When to consult a psychiatrist</h2><ul className="mt-6 grid gap-4">{condition.whenToConsult.map((item)=><li key={item} className="flex gap-3 text-sm leading-6 text-muted-foreground"><span className="mt-0.5 flex size-5 shrink-0 items-center justify-center rounded-full bg-primary/8 text-primary"><ArrowRight className="size-3"/></span>{item}</li>)}</ul></div></Reveal>
      ...
      <section className="container-page py-16 md:py-22">
        <div className="grid gap-10 lg:grid-cols-[.8fr_1.2fr] lg:gap-18"><Reveal><SectionHeading eyebrow="Frequently asked questions" title={`Questions about ${condition.name.toLowerCase()}`} description="General answers can orient you; your own plan requires an individual assessment."/></Reveal><Reveal delay={0.08} className="rounded-[1.7rem] border border-border bg-card px-6 shadow-card md:px-8"><FAQList faqs={condition.faqs}/></Reveal></div>
      </section>
      ...
      <AppointmentCTA title={`You are more than ${condition.name.toLowerCase()}.`} description="A consultation can turn a confusing pattern into a clearer, individual plan—without guaranteed-cure claims or one-size-fits-all treatment." />
    </>
  );
}
