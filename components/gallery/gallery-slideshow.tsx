...
export function GallerySlideshow({ compact = false }: { compact?: boolean }) {
  ...
  return (
    <div className="overflow-hidden rounded-[2rem] bg-transparent" onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)} onFocusCapture={() => setHovered(true)} onBlurCapture={(event) => { if (!event.currentTarget.contains(event.relatedTarget)) setHovered(false); }} onKeyDown={(event) => { if (event.key === "ArrowLeft") previous(); if (event.key === "ArrowRight") next(); if (event.key === " ") { event.preventDefault(); setPlaying((value) => !value); } }} onTouchStart={(event) => { touchStart.current = event.touches[0]?.clientX ?? null; }} onTouchEnd={(event) => { if (touchStart.current === null) return; const distance = (event.changedTouches[0]?.clientX ?? touchStart.current) - touchStart.current; if (Math.abs(distance) > 45) { if (distance > 0) previous(); else next(); } touchStart.current = null; }} tabIndex={0} aria-roledescription="carousel" aria-label="Clinic archive gallery">
      <div className={cn("relative overflow-hidden bg-[#071a2b]", compact ? "aspect-[16/10] min-h-80" : "aspect-[16/9] min-h-[24rem] md:min-h-[34rem]") }>
        <div key={`bg-${slide.id}`} className="gallery-backdrop absolute inset-0">
          <Image src={slide.src} alt="" fill sizes="100vw" className="scale-110 object-cover opacity-35 blur-2xl" aria-hidden="true" />
        </div>
        <div key={slide.id} className="gallery-slide absolute inset-0 p-4 md:p-7">
          <Image src={slide.src} alt={slide.alt} fill priority={index === 0} sizes="(max-width: 1024px) 100vw, 76rem" className="gallery-slide-image object-contain p-4 md:p-7" />
        </div>
        ...
        <div className="absolute left-4 top-4 rounded-full border border-white/20 bg-slate-950/55 px-3 py-1.5 text-[0.65rem] font-extrabold uppercase tracking-[0.1em] text-white backdrop-blur-md"><BiText en="Clinic archive" te="క్లినిక్ ఆర్కైవ్" /></div>
        <div className="absolute bottom-4 left-4 right-4 flex items-end justify-between gap-4 text-white md:bottom-6 md:left-7 md:right-7">
          ...
          <a href={slide.photoUrl || slide.albumUrl} target="_blank" rel="noreferrer" className="inline-flex size-10 shrink-0 items-center justify-center rounded-full border border-white/20 bg-white/10 backdrop-blur-md transition hover:bg-white/20" aria-label="View original Facebook album"><ExternalLink className="size-4" /></a>
        </div>
      </div>
      ...
      <div className="flex flex-col gap-4 px-1 pb-2 pt-5 md:px-2">
        ...
        <div className="flex gap-2 overflow-x-auto pb-1" role="tablist" aria-label="Choose gallery image">
